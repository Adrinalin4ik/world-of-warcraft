...
class Skybox extends THREE.Group {
  ...
  // per-batch global-sequence/local-animation tracks loop against (see `model.ts`'s own module doc).
  ...
  public update(camera: THREE.Camera, _mapID: number): void {
    ...
    // like every other sky shell in this client -- see model.ts's own "Placement" doc. Matches the
    // gradient dome's own `far * 0.9` (`sky/procedural/index.ts`) since this model REPLACES that dome.
