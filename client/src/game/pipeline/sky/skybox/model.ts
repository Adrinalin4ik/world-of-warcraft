 ...
 *    never 0 (opaque). The previous flat `transparent: false` `MeshBasicMaterial` for every batch
 *    painted each layer's raw RGB straight over whatever drew before it, including wherever a
 *    cloud/glow layer's texture is black at alpha 0 -- Nagrand's large black bands. Fixed by reading
 *    each batch's own `materialIndex` into `data.materials` and threading `blendingMode` through
 *    `buildSkyboxMeshes` -> `buildSkyboxMaterial`, which reuses this client's own WoW-blend-mode
 *    mapping (`applyBlendingModeToMaterial`, extracted from `pipeline/m2/material`'s
 *    `M2Material.applyBlendingMode`) rather than re-deriving GL blend factors here.
 *
 * 2. Fixing (1) alone traded the black bands for a solid white-out: most submeshes are drawn THREE
 *    times, once each with a `..._NIGHT_...`, `..._DAY_...` and `..._SUNSET_...` texture (e.g.
 *    submesh 6 draws `NAGRAND_NIGHT_CLOUDS01`, `NAGRAND_DAY_CLOUDS01` and `NAGRAND_SUNSET_CLOUDS01` in
 *    turn) -- three time-of-day variants of the same patch of sky, meant to be shown one at a time.
 *    What actually gates which one is visible right now is NOT static per-pixel alpha: it's the
 *    batch's own `vertexColorAnimationIndex` (-> `data.vertexColorAnimations[i].alpha`, an overall
 *    per-submesh-group fade, looping on a real `globalSequenceID` -- 0, 1 or 2, `data.sequences[id]`
 *    ms) multiplied by its `transparencyAnimationLookup` (-> `data.transparencyAnimationLookups[i]` ->
 *    `data.transparencyAnimations[j]`, the per-VARIANT night/day/sunset selector -- confirmed distinct
 *    per variant on the same submesh: NIGHT_CLOUDS01 lookup 2, DAY_CLOUDS01 lookup 5, SUNSET_CLOUDS01
 *    lookup 4). The transparency track is where this model does something easy to miss:
 *    EVERY entry in `data.transparencyAnimations` reports `globalSequenceID === -1` despite carrying a
 *    real multi-keyframe timeline spanning up to 320000ms -- because it is not on a GLOBAL sequence,
 *    it is on the model's own single LOCAL animation instead (`track.animationIndex` -> `data.animations[0]`,
 *    `length: 320000`, `probability: 32767` i.e. always active). A static environment model has no
 *    `AnimationManager` picking "Stand" vs "Walk" the way a creature does, so its one sequence is
 *    simply always looping, and `resolveAlphaTrack` uses that sequence's own `length` as the loop
 *    duration exactly as it uses `sequences[globalSequenceID]` for a real global sequence. Both
 *    mechanisms are `pipeline/m2/material`'s combiner comment's `sampled0.a * vertexColor.a *
 *    animatedTransparency`, just applied to a whole cloud LAYER at a time instead of one draw's own
 *    vertex/texture alpha. Get either evaluation wrong (e.g. treat the local-animation track as a
 *    static resting value) and every variant's alpha reads as its keyframe-0 value instead of its
 *    CURRENT one -- night/day/sunset all draw near-opaque at once and wash the sky white.
...
export type SkyboxBatch = {
  ...
  blendingMode: number;
  /** The batch's own M2 material `renderFlags`. Not currently read for anything -- every skybox batch
   * is already treated as unlit, unfogged, two-sided and non-depth-writing regardless of these bits
   * (see `buildSkyboxMaterial`'s doc) -- but kept on the batch so a future caller that needs a specific
   * bit (e.g. 0x04 no-backface-cull data validation) does not have to re-plumb the materials array. */
  renderFlags: number;
 ...
 * (`globalSequenceDurationMs > 0`) or a single constant resting value (`globalSequenceDurationMs === 0`,
 * `timestamps`/`values` both length 1) for a track with no global sequence and no animation player
 * driving it. See `resolveAlphaTrack`/`evaluateAlphaTrack`. */
export type AlphaTrack = {
  timestamps: number[];
  values: number[];
  globalSequenceDurationMs: number;
};

/**
 * Resolve one M2 `AnimationBlock<color16>` (`vertexColorAnimations[i].alpha` or a
 * `transparencyAnimations[j]` entry -- both decode to the same shape) into an [`AlphaTrack`].
 *
 ...
 * `vertexColorAnimations`' own mechanism on `NagrandSkyBox.m2` (idx 4-25, `globalSequenceID` 0-2).
 *
 ...
 * `length` is this track's loop duration -- the SAME 320000ms `sequences[0]` already carries, which is
 * exactly what the day/night/sunset cloud triplets need to actually alternate (see the module doc's
 * point 2; this is the piece that turned "everything visible, sky washes white" into the intended
 * cross-fade). A track with exactly one keyframe (truly static, e.g. `STARS.BLP`'s fixed alpha) needs
 * no duration at all and is left constant either way.
 */
export function resolveAlphaTrack(block: any, sequences: number[], animations: any[]): AlphaTrack | null {
  ...
  if (track.timestamps.length === 1) {
    return { timestamps: track.timestamps, values: track.values, globalSequenceDurationMs: 0 };
  }
  ...
  return { timestamps: track.timestamps, values: track.values, globalSequenceDurationMs: durationMs };
}

/**
 * Evaluate an [`AlphaTrack`] at `elapsedMs` (wall-clock ms since the skybox started loading -- see
 * `buildSkyboxMeshes`). A constant track (`globalSequenceDurationMs === 0`, from a block with no global
 * sequence) always returns its one value. A looping track wraps `elapsedMs` into
 * `[0, globalSequenceDurationMs)` and linearly interpolates between the bracketing keyframes, holding
 * flat before the first and after the last -- ordinary M2 keyframe semantics, just with the "current
 * time" coming from a free-running clock instead of a played animation's own cursor.
 */
export function evaluateAlphaTrack(track: AlphaTrack | null, elapsedMs: number): number {
  if (!track || track.timestamps.length === 0) {
    return 1;
  }
  if (track.timestamps.length === 1 || track.globalSequenceDurationMs <= 0) {
    return track.values[0];
  }

  const t = elapsedMs % track.globalSequenceDurationMs;
...
export async function loadSkyboxBatches(path: string): Promise<SkyboxBatch[]> {
  ...
  for (const batch of skinData.batches) {
    ...
    const renderFlags = material ? material.renderFlags : 0;
