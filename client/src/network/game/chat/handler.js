...
class ChatHandler extends EventEmitter {
  ...
  constructor(session) {
    ...
    // Listen for messages
    this.session.game.on('packet:receive:SMSG_GM_MESSAGECHAT', this.handleGmMessage.bind(this));
    this.session.game.on('packet:receive:SMSG_MESSAGE_CHAT', this.handleMessage.bind(this));
    this.session.game.on('packet:receive:SMSG_CHANNEL_NOTIFY', this.handleNotify.bind(this));
  }
